...
do {
  try {
    /opt/mssql-tools/bin/sqlcmd -S ".,1433" -U SA -P $env:SA_PASSWORD -Q "CREATE DATABASE [FurryHappiness]"
...
/var/opt/mssql/sqlpackage/sqlpackage /Action:Publish /SourceFile:FurryHappiness.Database.dacpac /TargetServerName:., 1433 /TargetDatabaseName:FurryHappiness /TargetUser:sa /TargetPassword:$env:SA_PASSWORD /Properties:IncludeCompositeObjects=True

/opt/mssql-tools/bin/sqlcmd -S ".,1433" -U SA -P $env:SA_PASSWORD -d "FurryHappiness" -i "/var/opt/mssql/backup/init.sql"